...
public class GameScene: SKScene {
    ...
    func setEndGameArea(){
        ...
        let sizeBlock = endGamePanel.frame.width*0.8/CGFloat("Game Over".characters.count)
        
        for (index, l) in "Game Over".characters.enumerated(){
    ...
    func generateLettersGame(){
        let teste  = possibleWords.flatMap({$0.text.characters})
    ...
    func sortPossibleWords(){
        possibleWords = possibleWords.sorted { $0.text.characters.count > $1.text.characters.count }
    }
    ...
    func verifyMatchCol(wordToFind: PossibleWord, col: Int){
        ...
        for index in (0 ... cols - 1).reversed(){
            if (letterArray?[col, index] != nil && (letterArray?[col, index]?.word.letters.count)! < wordToFind.text.characters.count ){
        ...
        for match in matches{
            ...
            for index in ((match) ... (match) + wordToFind.text.characters.count - 1).reversed(){
    ...
    func verifyMatchRow(wordToFind: PossibleWord, row: Int){
        ...
        for index in 0 ... cols - 1{
            if (letterArray?[index, row] != nil && (letterArray?[index, row]?.word.letters.count)! < wordToFind.text.characters.count){
        ...
        for match in matches{
            ...
            for index in (match) ... (match) + wordToFind.text.characters.count - 1{
